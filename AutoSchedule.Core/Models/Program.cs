...
namespace AutoSchedule.Core.Models
{
    public class Program
    {
        public static async Task Main(string[] args)
        {
            ...
            // Get data
            const string jsonPath = "./classList.json";

            //await DataProvider.WriteSessionsToJsonAsync(allLectures, jsonPath);

            //var classList = await DataProvider.ReadJsonToSessionsAsync<List<List<Session>>>(jsonPath);
            //var testList = await DataProvider.RequestSessionFromServer<Session>("http://10.30.111.145:5002/test_return_json");
            //var testJson = await DataProvider.RequestJsonFromServer("http://10.30.111.145:5002/test_return_json");
            //Console.WriteLine(testJson);

            var classList = allLectures;
            ...
            var scheduleToEnroll = classList.Where(c => enrollmentList.Contains(c.First().Name));
            ClassSelector.FindSchedules(scheduleToEnroll)
                .ForEach(s =>
                {
